...
public class TimeSVDRecommender extends BiasedMFRecommender {
    ...
    protected void setup() throws LibrecException {
        ...
        regBias = conf.getFloat("rec.global.regularization");
        ...
        for (MatrixEntry me : trainMatrix) {
            ...
            sum += hours((long) timeMatrix.get(u, i), minTimestamp);
            cnt++;
        }
        ...
        for (int iter = 1; iter <= numIterations; iter++) {
            ...
            for (MatrixEntry me : trainMatrix) {
                int u = me.row();
                    ...
                    // day t
                    int t = hours(timestamp, minTimestamp);
                    int bin = bin(t);
                    ...
                    for (int k = 0; k < numFactors; k++) {
                        ...
                        //System.out.println("Factor: " + k + "Loss: " + loss);
                    }
               // System.out.println("Pass: " + trainMatrix.column(0) + "Loss: " + loss);
            }
    ...
    /**
     * get the number of hours for a given time difference
     *
     * @param diff the difference between two time stamps
     * @return number of hours for a given time difference
     */
    private static int hours(long diff) {
        return (int) TimeUnit.MILLISECONDS.toHours(diff);
    }

    /**
     * get the number of hours between two timestamps
     *
     * @param t1 time stamp 1
     * @param t2 time stamp 2
     * @return number of hours between two timestamps
     */
    private static int hours(long t1, long t2) {
        return hours(Math.abs(t1 - t2));
    }
